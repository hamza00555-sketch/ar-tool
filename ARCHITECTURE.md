...
## 16. خاصية المسح ثلاثي الأبعاد (3D Scan)

تحوّل جسماً حقيقياً إلى مجسّم ثلاثي الأبعاد بالتصوير فقط — بدون ليدار وبدون تطبيق.

### الحقيقة التقنية (لماذا هذه المعمارية)
- **لا وصول لليدار من المتصفح** (ARKit فقط)، و**WebXR Depth لا يعمل على آيفون**. فالمسح عالي الجودة في المتصفح مستحيل مباشرةً.
- الطريقة الواقعية الوحيدة: **التقاط صور → إعادة بناء سحابية (photogrammetry) → GLB**. وهذا ما تفعله كل الأدوات (ar-code, KIRI, Polycam).
- إعادة البناء (COLMAP/OpenMVS) تحتاج **GPU وعمليات دقائق** — **مستحيلة على Vercel** (بلا GPU، دوال قصيرة). لذلك قُسّمت الميزة لطرفين يتصلان عبر عقد API.

### المعمارية (قائمة على المهام + عامل يسحب)
```
المتصفح (التقاط)  ──►  /api/scan (طابور)  ◄── يسحب ── العامل (خادمك بالـ GPU)
                            ▲                              │
                            └──── يُكمل بملف GLB ◄──────────┘
                                   (COLMAP → OpenMVS → GLB)
```
- **العامل يسحب (pull) ولا يُستقبَل (push):** يتصل بالتطبيق صادراً فقط، فيعمل خلف NAT/إنترنت منزلي بلا منافذ واردة.
- **الناتج يصبح تجربة «مجسّم» عادية** — فيرث كل خط الأنابيب (Quick Look، الزينة، الصوت). لا نولّد USDZ في العامل: المجسّم ثابت، فتحويل model-viewer على الجهاز GLB→USDZ كافٍ (لا أنيميشن ليُحذف).

### الأجزاء
| الملف | الدور |
|---|---|
| `supabase/migrations/0005_scan_jobs.sql` | جدول `scan_jobs` + دالة `claim_scan_job()` بـ **SKIP LOCKED** (عاملان لا يأخذان نفس المهمة) |
| `src/app/scan/page.tsx` | التقاط موجّه: كاميرا + حلقة تأطير + التقاط تلقائي للإطارات + تلميحات جودة + رفع |
| `src/app/scan/[id]/page.tsx` | شاشة المعالجة: تسأل الحالة كل 3ث، وتحوّل للتجربة عند الجهوزية |
| `src/app/api/scan/*` | إنشاء مهمة / استعلام حالة / **claim** و**complete** للعامل (بحماية `WORKER_TOKEN`) |
| `worker/run.py` | العامل الحقيقي: يسحب مهمة → COLMAP (SfM) → OpenMVS (كثافة/شبكة/كساء) → `obj2gltf` → يرفع → يُكمل |
| `worker/Dockerfile` | صورة CUDA فيها COLMAP+OpenMVS+obj2gltf |
| `scripts/mock-scan-worker.mjs` | **عامل وهمي** يرجّع النموذج المرفق بدل التصوير — لاختبار الحلقة كاملة بلا GPU |

### عقد رفع نتائج العامل (ذكاء مهم)
العامل **يرفع ناتجه عبر نفس مسار الرفع العام** الذي يستخدمه المتصفح (`/api/upload/sign` ثم PUT) — فلا يتجاوز حد Vercel 4.5MB (يذهب مباشرة للتخزين)، ولا يحتاج مفاتيح Supabase. نقاط `claim`/`complete` فقط محمية بـ `WORKER_TOKEN` (سرّ مشترك بين التطبيق والعامل).

### التشغيل
1. عيّن `WORKER_TOKEN` في متغيرات Vercel، وأعطِ نفس القيمة للعامل.
2. شغّل العامل على خادم GPU: `docker run --gpus all -e APP_URL=… -e WORKER_TOKEN=… holoform-worker` (راجع `worker/README.md`).
3. بلا `WORKER_TOKEN`: الالتقاط والطابور يعملان، لكن لا شيء يعالج (نقاط العامل ترجع 503) — مفيد للتجربة.

### فخاخ خاصة بالمسح
| الفخ | الحل |
|---|---|
| GPU/عمليات طويلة على Vercel | عامل منفصل يسحب المهام؛ Vercel للطابور فقط |
| حد 4.5MB لرفع ناتج ضخم | العامل يرفع بروابط موقّعة مباشرة للتخزين |
| عاملان يأخذان نفس المهمة | `claim_scan_job()` بـ `FOR UPDATE SKIP LOCKED` |
| العامل خلف NAT | نموذج السحب (لا منافذ واردة) |
| أجسام لامعة/سادة | COLMAP يفشل بملامح قليلة → رسالة واضحة + تلميحات جودة في الواجهة |
| اختبار بلا GPU | العامل الوهمي + اختبار E2E كامل للحلقة |

> **الجودة النهائية للمجسّم** تحتاج خادم GPU حقيقياً للتحقق (COLMAP/OpenMVS لا يعملان headless هنا) — تماماً مثل Quick Look الذي يحتاج آيفون. المنطق والعقد والحلقة كلها مُختبَرة بالعامل الوهمي (16/16 فحصاً).
